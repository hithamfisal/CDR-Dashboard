...
export default function App() {
  ...
  const exportChartWorkbookXlsx = useCallback((fileName: string, datasets: Record<string, ChartExportDataset>) => {
    void (async () => {
      const ExcelJS = await loadExcelJS();
      const workbook = new ExcelJS.Workbook();
      workbook.creator = "CDR Dashboard";
      ...
      const border = { top: { style: "thin" as const }, left: { style: "thin" as const }, bottom: { style: "thin" as const }, right: { style: "thin" as const } };
      const headerFill = { type: "pattern" as const, pattern: "solid" as const, fgColor: { argb: "FF0F5F8F" } };
      const titleFill = { type: "pattern" as const, pattern: "solid" as const, fgColor: { argb: "FF082033" } };
      ...
      Object.entries(datasets).forEach(([title, dataset]) => {
        ...
        const chartRows = dataset.rows.map((row) => row.map((cell, index) => numericColumns.includes(index) ? toNumericValue(cell) : cell));
      ...
      downloadBlob(fileName, output instanceof Blob ? output : new Blob([output], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
    })();
  }, [exportTitle]);
  ...
  const exportSingleChartDataXlsx = useCallback((title: string, dataset: ChartExportDataset) => {
    exportChartWorkbookXlsx(`${fileSlug(exportTitle(title))}.xlsx`, { [title]: dataset });
  }, [exportChartWorkbookXlsx, exportTitle]);

  const chartExportItems = useMemo(() => Object.entries(chartExportDatasets).map(([title, dataset]) => ({
    label: title,
